...
(function() {
    ...
    var CanvasInput = window.CanvasInput = function(o) {
        ...
        self._padding_w = o.padding_w >= 0 ? o.padding_w : 5;
        self._padding_h = o.padding_h >= 0 ? o.padding_h : 5;
        ...
        self._shadowCanvas.setAttribute('width', self._width + self._padding_w * 2);
        self._shadowCanvas.setAttribute('height', self._height + self._padding_h * 2);
        ...
        if (self._canvas) {
            ...
            self._canvasMouseMove = function(e) {
                e = e || window.event;
                self.mousemove(e, self);
            };
            self._canvasMouseDown = function(e) {
                e = e || window.event;
                self.mousedown(e, self);
            };
            self._canvasMouseUp = function(e) {
                e = e || window.event;
                self.mouseup(e, self);
            };

            self._canvas.addEventListener('mousemove',self._canvasMouseMove , false);
            self._canvas.addEventListener('mousedown',self._canvasMouseDown , false);
            self._canvas.addEventListener('mouseup',self._canvasMouseUp , false);
        }

        // setup a global mouseup to blur the input outside of the canvas
        self.autoBlur = function(e) {
            e = e || window.event;
        ...
        window.addEventListener('mouseup', self.autoBlur, true);
        window.addEventListener('touchend', self.autoBlur, true);
        ...
        self._hiddenInput.style.zIndex = 999999;
        ...
        self._hiddenInput.onfocus = function(e){
            self._hasFocus = false;
            self.focus();
            setTimeout(function(){
                self.selectText();
                self._hiddenInput.scrollIntoView(true)
            },200)
        };

        self._hiddenInput.onblur = function(){
            self.blur();
            self._hiddenInput.scrollIntoView(true)
        }
    ...
    CanvasInput.prototype = {
        ...
        padding: function(w,h) {
            ...
            if (typeof data !== 'undefined') {
                ...
                return {w:self._padding_w , h:self._padding_h};
            }
        },
        ...
        render: function() {
            ...
            self._hiddenInput.style.left = ((self._x + self._extraX + (self._canvas ? self._canvas.offsetLeft : 0)) / zoom) + 'px';
            self._hiddenInput.style.top = ((self._y + self._extraY + (self._canvas ? self._canvas.offsetTop : 0)) / zoom) + 'px';
            ...
            self._drawTextBox(function() {
                ...
                // draw the selection
                var paddingBorder = self._padding_h + self._borderWidth + self.shadowT;
                var paddingBorder_w = self._padding_w + self._borderWidth + self.shadowT;

                if(self._align == 1){
                    paddingBorder_w = ( (self._width + self._borderWidth + self.shadowT + self._padding_w) - tw) / 2;
                }
                ...
                if (self._selection[1] > 0) {
                    ...
                    ctx.fillRect(paddingBorder_w + selectOffset, paddingBorder, selectWidth, self._height);
                }

                // draw the cursor
                if (self._cursor) {
                    var cursorOffset = self._textWidth(text.substring(0, self._hiddenInput.selectionStart));
                    ctx.fillStyle = self._fontColor;
                    ctx.fillRect(paddingBorder_w + cursorOffset, paddingBorder, 1, self._height);
                }

                // draw the text
                var textX = paddingBorder_w + self._borderWidth + self.shadowL,
        ...
        destroy: function() {
            ...
            self._renderCtx.clearRect(0, 0, self._renderCtx.canvas.width, self._renderCtx.canvas.height);
            //document.body.removeChild(self._renderCanvas);
            ...
            window.removeEventListener('mouseup', self.autoBlur, false);
            window.removeEventListener('touchend', self.autoBlur, false);

            self._canvas.removeEventListener('mousemove',self._canvasMouseMove , false);
            self._canvas.removeEventListener('mousedown',self._canvasMouseDown , false);
            self._canvas.removeEventListener('mouseup',self._canvasMouseUp , false);
        },
        ...
        _calcWH: function() {
            ...
            // calculate the full width and height with padding, borders and shadows
            self.outerW = self._width + self._padding_w * 2 + self._borderWidth * 2 + self.shadowW;
            self.outerH = self._height + self._padding_h * 2 + self._borderWidth * 2 + self.shadowH;
        },
        ...
        _updateHiddenInput: function() {
            var self = this;
            if(self._align == 1){
                self._hiddenInput.style.textAlign = "center";
            }
            self._hiddenInput.style.fontSize = self._fontSize + "px";
            self._hiddenInput.style.padding = self._padding_h + "px " + self._padding_w + "px " + self._padding_h + "px " + self._padding_w + "px";
        ...
        _overInput: function(x, y) {
            var self = this,
                ...
                xRight = x <= self._x + self._extraX + self._width + self._padding_w * 2,
                yTop = y >= self._y + self._extraY,
                yBottom = y <= self._y + self._extraY + self._height + self._padding_h * 2;
